    ...
    while cap.isOpened():
        ...
        if not game_over:
            if not pause:
                ...
                # Falling into player1's court and player2 scoring
                if ball_y > h - ball_radius and ball_x < w / 2 - net_thickness:
                    scorePlayer2 += 1  # Player2 score alması
                    ball_restart()

                # Falling into player2's court and player1 scoring
                if ball_y > h - ball_radius and ball_x > w / 2 - net_thickness:
                ...
                if results.multi_hand_landmarks:
                    for hand_landmarks in results.multi_hand_landmarks:
                        ...
                        # Ball and Hand Collision Control
        ...
        # Hand detection and button pressing control
        if results.multi_hand_landmarks:
            for hand_landmarks in results.multi_hand_landmarks:
                mp_drawing.draw_landmarks(frame, hand_landmarks, mp_hands.HAND_CONNECTIONS)
                finger_x = hand_landmarks.landmark[mp_hands.HandLandmark.INDEX_FINGER_TIP].x * w
